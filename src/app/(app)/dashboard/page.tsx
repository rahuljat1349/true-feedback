...
const page = () => {
  ...
  const fetchAcceptMessage = useCallback(async () => {
    ...
    try {
      const response = await axios.get<ApiRespons>("/api/accept-messages");
  ...
  const fetchMessages = useCallback(
    async (refresh: boolean = true) => {
      ...
      setIsSwitchLoading(false);
      try {
        const response = await axios.get<ApiRespons>("/api/get-messages");
        setMessages(response.data.messages || []);
        if (refresh) {
          toast({
            title: "Refreshed Messages",
            description: "You are seeing latest messages!",
          });
        }
      } catch (error) {
        const axiosError = error as AxiosError<ApiRespons>;
  ...
  const handleSwitchChange = async () => {
    try {
      const response = await axios.post<ApiRespons>("/api/accept-messages", {
